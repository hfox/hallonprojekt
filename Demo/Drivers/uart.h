...
/* Semaphores for ensuring only one task can read and one can write at any
 * given time. Defined here so that {Write,Read}UART may be put here for
 * inlining later on. */
extern xSemaphoreHandle ReadSemaphore;
extern xSemaphoreHandle WriteSemaphore;
